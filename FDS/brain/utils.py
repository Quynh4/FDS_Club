...
def plot_from_img_path(rows, columns, list_img_path, list_mask_path):
    fig = plt.figure(figsize=(12, 12)) # Khởi tạo một đối tượng hình ảnh matplotlib với kích thước là 12x12
    for i in range(1, rows * columns + 1):  # Thực hiện vòng lặp để vẽ từng hình ảnh
        fig.add_subplot(rows, columns, i)  # Thêm subplot vào hình ảnh
        img_path = list_img_path[i]  # Lấy đường dẫn hình ảnh trong danh sách đường dẫn
        mask_path = list_mask_path[i]  # Lấy đường dẫn mặt nạ tương ứng trong danh sách đường dẫn
        image = cv2.imread(img_path)  # Đọc hình ảnh từ đường dẫn
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Chuyển đổi không gian màu của hình ảnh BGR sang RGB
        mask = cv2.imread(mask_path)  # Đọc mặt nạ từ đường dẫn
        plt.imshow(image)  # Hiển thị hình ảnh
        plt.imshow(mask, alpha=0.4)  # Hiển thị mặt nạ với độ trong suốt là 0.4
    plt.show()  # Hiển thị tất cả các hình ảnh và mặt nạ
